...
class TestResolveOnnxPath:
    def test_adds_onnx_suffix(self, tmp_path):
        ...
        from voice.tts.piper import _resolve_onnx_path
...
class TestGetVoice:
    def test_loads_voice_on_first_call(self, tmp_path):
        ...
        with patch("voice.tts.piper._voice_cache", {}), \
             patch("voice.tts.piper.PiperVoice") as MockPV:
            MockPV.load.return_value = mock_voice
            from voice.tts.piper import _get_voice
    ...
    def test_returns_cached_voice_on_second_call(self, tmp_path):
        ...
        with patch("voice.tts.piper._voice_cache", cache), \
             patch("voice.tts.piper.PiperVoice") as MockPV:
            MockPV.load.return_value = mock_voice
            from voice.tts.piper import _get_voice
...
class TestWarmup:
    def test_warmup_silences_file_not_found(self):
        ...
        with patch("voice.tts.piper._resolve_onnx_path",
                   side_effect=FileNotFoundError("no model")):
            from voice.tts.piper import warmup
    ...
    def test_warmup_loads_voice_when_model_present(self, tmp_path):
        ...
        with patch("voice.tts.piper._resolve_onnx_path", return_value=onnx_path), \
             patch("voice.tts.piper._get_voice", return_value=mock_voice) as mock_get:
            from voice.tts.piper import warmup
...
class TestSynthesizeSpeech:
    ...
    def test_writes_wav_file(self, tmp_path):
        ...
        with patch("voice.tts.piper._HAS_PIPER_API", True), \
             patch("voice.tts.piper._resolve_onnx_path", return_value=tmp_path / "v.onnx"), \
             patch("voice.tts.piper._get_voice", return_value=mock_voice):
            from voice.tts.piper import synthesize_speech
    ...
    def test_raises_when_piper_not_installed(self):
        with patch("voice.tts.piper._HAS_PIPER_API", False):
            from voice.tts.piper import synthesize_speech
    ...
    def test_speed_sets_length_scale(self, tmp_path):
        ...
        with patch("voice.tts.piper._HAS_PIPER_API", True), \
             patch("voice.tts.piper._resolve_onnx_path", return_value=tmp_path / "v.onnx"), \
             patch("voice.tts.piper._get_voice", return_value=mock_voice), \
             patch("voice.tts.piper.SynthesisConfig") as MockSC:
            from voice.tts.piper import synthesize_speech
