...
class TestGetModel:
    def test_creates_model_on_first_call(self):
        ...
        with patch("voice.stt.whisper._model_cache", {}), \
             patch("voice.stt.whisper.WhisperModel", return_value=mock_model) as MockWM:
            from voice.stt.whisper import _get_model
    ...
    def test_returns_cached_model_on_second_call(self):
        mock_model = MagicMock()
        cache = {}
        with patch("voice.stt.whisper._model_cache", cache), \
             patch("voice.stt.whisper.WhisperModel", return_value=mock_model) as MockWM:
            from voice.stt.whisper import _get_model
    ...
    def test_different_params_create_separate_models(self):
        ...
        cache = {}
        with patch("voice.stt.whisper._model_cache", cache), \
             patch("voice.stt.whisper.WhisperModel", side_effect=models):
            from voice.stt.whisper import _get_model
...
class TestTranscribeAudio:
    ...
    def test_joins_segments(self, tmp_path):
        ...
        with patch("voice.stt.whisper._get_model", return_value=mock_model):
